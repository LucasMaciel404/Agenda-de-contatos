...
// METODOS STATICOS
Contato.buscaPorId = async function (myId) {
    if(typeof myId !== 'string') return;
    const contato = await ContatoModel.findById(myId);
    return contato;
}
...
Contato.delete = async function (myId) {
    if(typeof myId !== 'string') return;
    const contato = await ContatoModel.findOneAndDelete(myId);
    return contato;
}
